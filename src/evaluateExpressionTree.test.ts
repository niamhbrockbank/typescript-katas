...
test.skip("invalid arguments are handled", () => {
  //@ts-ignore
  expect(evaluateTree({ op: "twelve", a: 3, b: 5 })).toBe(
    "invalid input, please use the Expression structure"
  );
  //@ts-ignore
  expect(evaluateTree("twelve")).toBe(
    "invalid input, please use the Expression structure"
  );
});
...
test("three tier trees and above evaluate to correct number", () => {
  expect(
    evaluateTree({
      op: "multiply",
      a: { op: "subtract", a: 7, b: 5 },
      b: { op: "divide", a: { op: "add", a: 5, b: 10 }, b: 3 },
    })
  ).toBe(10);
});
